...
function EnhancedTableToolbar(props) {
  ...
  return (
    <Toolbar sx={{ maxWidth: 1050, minWidth: 1050 }}>
      ...
      {authCtx.authorities.includes("demand:write") && (
        <Button
          sx={{ flex: "1 1 60%" }}
          component={Link}
          to={"newDemand"}
    ...
    <Box>
      {openDialog && (
        <DeleteEntryDialog
          ...
          entryId={objectId}
          text={
            "Czy napewno chcesz usunąć zapotrzebowanie " + objectName + " ?"
          }
        />
      )}
      <Paper sx={{ width: "100%", minHeight: "53vh", mb: 2 }}>
                ...
                .map((row, index) => {
                  return (
                    <TableRow key={`${row}${index}`}>
                      ...
                      <TableCell align="left">
                        <Typography>{row.demandName}</Typography>
                      </TableCell>
                      <TableCell align="center" width={"170"}>
                        <Typography
                          sx={{
                            ...
                            padding: "3px 10px",
                            backgroundColor:
                              (row.demandStatus === "Nowy" && "white") ||
                              (row.demandStatus === "Zapotrzebowany" &&
                                "yellow") ||
                              (row.demandStatus === "Zamówiony" && "orange") ||
                              (row.demandStatus === "Dostarczony" && "green") ||
                              (row.demandStatus === "Anulowany" && "red"),
                          }}
                        >
                          {row.demandStatus}
                        </Typography>
                        ...
                        >
                          {authCtx.authorities.includes("ROLE_ADMIN") && (
                            <Button
                              ...
                              onClick={() =>
                                openInDialog(row.id, row.demandName)
                              }
                            >
                              Usuń
                            </Button>
        ...
        <TablePagination
          ...
          labelRowsPerPage="Zapotrzebowań na stronę:"
          labelDisplayedRows={({ from, to, count }) =>
            `Wyświetlono ${from}-${to} zapotrzebowań z ${count}`
          }
        />
      </Paper>
    </Box>
